	...
	<view>
		...
		<view class="header">
			...
			<view class="before" :style="{ opacity: 1 - afterHeaderOpacity, zIndex: beforeHeaderzIndex }">
				...
				<!-- <view class="icon-btn">
					<view class="icon tongzhi" @tap="toMsg">消息</view>
					<view class="icon cart" @tap="joinCart">购物车</view>
				</view> -->
			</view>
			<!-- 头部-滚动渐变显示 -->
		...
		<view class="footer">
			<view class="icons">
				...
				<view class="box" @tap="keep">
					<!-- <view class="iconfont" :class="[isKeep?'shoucangsel':'shoucang']">&#xe62e;</view> -->
					<view class="iconfont" v-if="!isKeep">&#xe62e;</view>
					<view class="iconfont" v-if="isKeep">&#xeca7;</view>
		...
		data() {
			return {
				...
				goodsData: {
					...
					service: [{
							name: "新鲜安全",
							description: "绿色无污染的健康产品"
						},
						{
							name: "极速发货",
							description: "保障在48小时内发货"
						},
						{
							name: "不支持7天退换",
							description: "此商品不支持天无理由退换服务"
						}
					],
					spec: [],
					comment: {
				...
				isKeep: false, //收藏
				//商品描述html
				descriptionStr: '<div style="text-align:center;width:750rpx;"><img width="100%" src="https://ae01.alicdn.com/kf/HTB1t0fUl_Zmx1VjSZFGq6yx2XXa5.jpg"/><img width="100%" src="https://ae01.alicdn.com/kf/HTB1LzkjThTpK1RjSZFKq6y2wXXaT.jpg"/><img width="100%" src="https://ae01.alicdn.com/kf/HTB18dkiTbvpK1RjSZPiq6zmwXXa8.jpg"/></div>'
			};
		},
		onLoad(options) {
			// #ifdef MP
		...
		methods: {
			...
			keep() {
				...
				if(this.isLogin){
					...
					if (that.isKeep == false) {
						...
						request.get('collectController/insert', data).then((res) => {
							if (res.code == 1) {
								uni.showToast({
									title: "已收藏",
									duration: 500
						...
						request.get('collectController/cancelCollect', params).then((res) => {
							uni.showToast({
								title: "取消收藏",
								duration: 500
							})
			...
			joinCart() {
				if(this.isLogin){
					...
					let data = {
						userId: this.userId,
						goodsId: this.id,
						goodsSn: this.goodsData.goodsSn,
						goodsName: this.goodsData.name,
						num: this.goodsData.number,
						price: this.goodsData.price,
						specifications: spec,
						picUrl: this.goodsData.picUrl
					}
					data = JSON.stringify(data);
					console.log("************************");
					console.log(data);
					request.post('goodsCartController/insert', data).then((res) => {
						console.log(res);
					})
					uni.showToast({
						title: "已加入购物车"
			...
			initData() {
				...
				request.get('goodsController/getGoodsList', data).then((res) => {
					// console.log(res)
					this.goodsData.id = res.list[0].id;
					this.goodsData.goodsSn = res.list[0].goodsSn;
					this.goodsData.picUrl = res.list[0].picUrl;
					this.goodsData.name = res.list[0].name;
					this.goodsData.price = res.list[0].retailPrice;
					this.descriptionStr = res.list[0].detail.replace(/\<img/gi,   '<img class="rich-img" ');
					this.swiperList = res.list[0].gallery;
					this.goodsData.goodsSn = res.list[0].goodsSn;
				...
				request.get('goodsSpecificationController/getByGoodsId', params).then((res) => {
					if(res.code == 1){
						console.log(res.data);
						res.data.forEach(v => {
							this.goodsData.spec.push(v.value)
							console.log(v);
						})
					}else{
						uni.showToast({
				...
				request.get('collectController/isExist', query).then((res) => {
					if (res.code == 1) {
						this.isKeep = true;
					} else {
						this.isKeep = false;
