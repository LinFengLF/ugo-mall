<template>
	<view>
		...
		<view class="goods-list">
			<view class="tis" v-if="!isLogin">您还没有登录哦~</view>
			<view class="tis" v-if="goodsList.length==0 && isLogin">购物车是空的哦~快去购买吧</view>
				...
				<view class="carrier" :class="[theIndex==index?'open':oldIndex==index?'close':'']" @touchstart="touchStart(index,$event)"
				 @touchmove="touchMove(index,$event)" @touchend="touchEnd(index,$event)">
					...
					<view class="checkbox-box" @tap="selected(index)">
						<view class="checkbox">
							<view :class="[row.checked == 1?'on':'']"></view>
						</view>
					</view>
					<!-- 商品信息 -->
							...
							<view class="price-number">
								...
								<view class="number">
									<view class="sub" @tap.stop="sub(index)">
										<view style="font-size: 20px;justify-content: middle;text-align: center;" class="icon jian">-</view>
									</view>
									<view class="input">
										<!-- @tap.stop="discard" -->
									...
									<view class="add" @tap.stop="add(index)">
										<view style="font-size: 20px;justify-content: middle;text-align: center;" class="icon jia">+</view>
									</view>
								</view>
							</view>
						</view>
	...
	export default {
		...
		methods: {
			initData() {
				...
				request.get('goodsCartController/getListByUserId', data).then((res) => {
					if (res.code == 1) {
						that.goodsList = res.data;
						console.log(res);
						console.log(that.goodsList);
						// that.goodsList.forEach(v => {
						// 	that.sumPrice += Number(v.price);
						// })
					} else {
						that.goodsList = [];
						// uni.showToast({
			...
			toGoods(e) {
				...
				uni.navigateTo({
					url: '/pages/detail/detail?id=' + e.goodsId
				});
			},
			//跳转确认订单页面
			toConfirmation() {
				...
				for (let i = 0; i < len; i++) {
					if (this.goodsList[i].checked == 1) {
						let goods = {
							id: this.goodsList[i].goodsId,
							img: this.goodsList[i].picUrl,
							name: this.goodsList[i].goodsName,
							spec: '规格:' + this.goodsList[i].specifications,
							price: this.goodsList[i].price,
							number: this.goodsList[i].num
						}
				...
				uni.showModal({
					...
					success(res) {
						if (res.confirm) {
							...
							request.get('goodsCartController/deleteByBatch', data).then((res) => {
								console.log(res);
							})
							console.log('用户点击确定');
						} else if (res.cancel) {
			...
			selected(index) {
				// debugger
				this.goodsList[index].checked = this.goodsList[index].checked == 1 ? 0 : 1;
			...
			allSelect() {
				...
				let arr = [];
				for (let i = 0; i < len; i++) {
					this.goodsList[i].checked = this.isAllselected ? 0 : 1;
