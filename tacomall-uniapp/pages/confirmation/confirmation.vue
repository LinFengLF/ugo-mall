	...
	<view>
		...
		<view class="addr" @tap="selectAddress">
			...
			<view class="right">
				...
				<view class="addres">
					{{recinfo.address.region.province}}{{recinfo.address.region.country}}{{recinfo.address.region.city}}
...
<script>
	...
	export default {
		...
		onShow() {
			// this.userId = uni.getStorageInfoSync("isLogin").userId;
			//页面显示时，加载订单信息
			// debugger
			uni.getStorage({
				key: 'buylist',
		...
		methods: {
			...
			toPay() {
				...
				debugger
				let orderInfo = {
					address:this.recinfo.address.region.province + this.recinfo.address.region.country + 
								this.recinfo.address.region.city + this.recinfo.address.detailed,
					note:this.note,
			...
			initAddress() {
				...
				request.get('addressController/getListByUserId', data).then((res) => {
					if (res.code == 1) {
						res.data.forEach(v => {
							// console.log(v);
							if (v.isDefault == 1) {
								console.log(v);
								that.recinfo.id = v.id;
								that.recinfo.name = v.name;
								that.recinfo.tel = v.tel;
								that.recinfo.address.region.province = v.province;
								that.recinfo.address.region.city = v.city;
								that.recinfo.address.region.country = v.county;
								that.recinfo.address.detailed = v.addressDetail;
								that.recinfo.isDefault = v.isDefault == 1 ? true : false;
							}
							// console.log(v);
						})
					} else {
