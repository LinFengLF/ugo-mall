<template>
	<view>
		<view class="tabr" :style="{top:headerTop}">
			<view :class="{on:typeClass=='goods'}" @tap="switchType('goods')">以下是您收藏的全部商品({{goodsList.length}})</view>
		...
		<view class="list">
			...
			<view class="sub-list goods" :class="subState">
				...
				<view class="row" @click="productDetail(row)" v-for="(row,index) in goodsList" :key="index">
					<!-- 删除按钮 -->
					<view class="menu" @tap.stop="deleteCollection(row.id,goodsList)">
						...
						<view class="goods-info">
							...
							<view class="info">
								...
								<view class="price-number">
									...
									<view class="price">￥{{row.retailPrice}}</view>
								</view>
							</view>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			initData() {
				...
				request.get('collectController/getUserCollection', data).then((res) => {
			...
			//删除商品
			deleteCollection(id, List) {
				// debugger
				let data = {
					valueId: id
				}
				request.get('collectController/deleteByValueId',data).then((res) => {
					console.log(res);
				})
				let len = List.length;
				for (let i = 0; i < len; i++) {
